# print out initialized parameters for this run - paste the output to the start
cat("
# ------------------------------------------------------------
...
# p1_sample <- function(n, pt_neg, pt_pos, pt_wt, pt_ctrl){ # borrow parameter used from CRISPulator - could re-parametrize from real data
#   p1 = c(rtruncnorm(round(pt_neg*n), a = -1, b = -0.025, mean = mu_neg, sd = sd_neg), # negative phenotype - essential genes
#          rtruncnorm(round(pt_pos*n), a = 0.025, b = 1, mean = mu_pos, sd = sd_pos), # positive phenotype
#          rtruncnorm(round(pt_wt*n), a = -0.5, b = 0.5, mean = 0, sd = sd_wt), # wildtype - non-essential genes
#          rep(0, round(pt_ctrl*n)))
#   return(p1)
# }
...
run_replicate <- function(replicate_name, cell_lib_guide0) {
  ...
  while (i.bottleneck < n.bottlenecks & i.iterations < n.iterations) {
    ...
    # #### Optional 1: single-cell-level growth
    # # Expand data by each single cell
    # expand_counts <- rep.int(cell_lib_guide1$guide_id, cell_lib_guide1$counts_guide_t1)
    # expanded_data <- data.table(guide_id = expand_counts)
    # expanded_data$rel_freq_guide_t1 <- cell_lib_guide1$counts_guide_t1[match(expanded_data$guide_id, cell_lib_guide1$guide_id)] / length(expand_counts)
    # 
    # expanded_data$counts_growth_t1 <- vapply(seq_len(nrow(expanded_data)), function(i) {
    #   idx <- match(expanded_data$guide_id[i], cell_lib_guide1$guide_id)
    #   if (is.na(idx)) return(0)
    #   cell_grow(round(expanded_data$rel_freq_guide_t1[idx] * n_construct), param_matrix[idx, ])
    # }, numeric(1))
    # 
    # out_t1 <- expanded_data[, .(counts_guide_t1 = sum(counts_growth_t1)), by = guide_id]
    # cell_lib_guide1$counts_guide_t1 <- out_t1$counts_guide_t1[match(cell_lib_guide1$guide_id, out_t1$guide_id)]
    # cell_lib_guide1$counts_guide_t1[is.na(cell_lib_guide1$counts_guide_t1)] <- 0
    
    #### Optional 2: bulk-level growth
  ...
  # Final growth after all bottlenecks
  #### Optional 1: single-cell-level growth
  # param_matrix <- as.matrix(cell_lib_guide1[, c("p0y_p", "p1y_p", "p2y_p", "p3y_p")])
  # expand_counts <- rep.int(cell_lib_guide1$guide_id, cell_lib_guide1$counts_guide_t1)
  # expanded_data <- data.table(guide_id = expand_counts)
  # 
  # expanded_data$counts_growth_t2 <- vapply(seq_len(nrow(expanded_data)), function(i) {
  #   idx <- match(expanded_data$guide_id[i], cell_lib_guide1$guide_id)
  #   if (is.na(idx)) return(0)
  #   cell_grow(1, param_matrix[idx, ])
  # }, numeric(1))
  # 
  # out_t2 <- expanded_data[, .(counts_guide_t2 = sum(counts_growth_t2)), by = guide_id]
  # cell_lib_guide1$counts_guide_t2 <- out_t2$counts_guide_t2[match(cell_lib_guide1$guide_id, out_t2$guide_id)]
  # cell_lib_guide1$counts_guide_t2[is.na(cell_lib_guide1$counts_guide_t2)] <- 0
  
  #### Optional 2: bulk-level growth
  # cell_lib_guide1$counts_guide_t2 <- vapply(seq_len(nrow(cell_lib_guide1)), function(i) {
  #   cell_grow(cell_lib_guide1$counts_guide_t1[i], param_matrix[i, ])
  # }, numeric(1))
  # 
